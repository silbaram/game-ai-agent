---
name: game-mvp-scope
description: 게임 기획을 실제로 만들 수 있는 MVP로 축소할 때 사용한다. pillar 기여, 복잡도, 의존성을 기준으로 모든 기능을 Must / Later / Cut으로 분류한다. 규칙 초안 작성 후, 스프레드시트나 구현 전에 호출한다. 이 단계는 선택 사항이 아니며, 생략하면 출시되지 못할 프로젝트로 범위가 부풀어진다.
---
