...
description: MVP 범위가 확정된 뒤 규칙 문서를 CSV/XLSX 게임 데이터 테이블, schema 문서, 초안 record, 검증 규칙으로 변환할 때 사용한다. spreadsheet-architect가 호출한다.
---
...
규칙 문서의 `필요한 데이터 테이블(Required Data Tables)`을 기준으로 삼고, MVP 범위 밖의 데이터는 만들지 않는다.
...
1. 모든 규칙 파일의 `필요한 데이터 테이블(Required Data Tables)` 섹션을 수집한다.
