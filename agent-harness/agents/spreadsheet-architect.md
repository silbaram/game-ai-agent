...
description: MVP 범위 검토 후 게임 규칙을 schema, 샘플 record, 검증 메모가 있는 CSV/XLSX 데이터 테이블로 변환할 때 사용한다.
...
- `game-design/rules/*.md`의 `필요한 데이터 테이블(Required Data Tables)` 섹션을 읽는다
...
2. 각 규칙 파일의 `필요한 데이터 테이블(Required Data Tables)` 섹션에서 테이블 이름과 컬럼 수집.
3. 컬럼별로 아래 속성을 결정:
    - 이름 (snake_case)
...
```text
spreadsheet-architect를 사용한다.

참조:
  - game-design/rules/*.md
  - game-design/mvp-scope.md (있다면)

작업:
  1. 모든 규칙 파일에서 데이터 테이블 요구사항 수집
  2. game-design/spreadsheets/SCHEMA.md를 먼저 작성
  3. 각 10행의 초안 데이터를 가진 csv 파일 생성
  4. game-master.xlsx로 묶기
```
