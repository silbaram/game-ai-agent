...
1. 대상 시스템의 **Purpose**를 한 문단으로 재정리한다.
    - 규칙 문서의 Purpose를 참조하되, 개발 관점으로 재서술 (어떤 코드 컴포넌트가 필요한지)
2. **User Flow**를 그린다.
...
8. **Non-functional requirements**를 표시한다.
    - 성능 기준 (예: "턴 처리 <100ms")
    - 저장 주기
    - 동시성 고려사항 (있다면)
...
- [ ] Purpose가 개발 관점으로 재서술됨
- [ ] User Flow에 정상 + 실패 두 경로 있음
- [ ] Data Model에 런타임 타입이 코드로 표현됨
- [ ] UI Screens에 각 화면의 요소 나열됨
- [ ] Edge Cases가 개발 관점에서 정리됨
- [ ] Test Cases에 Given/When/Then 형식 사용
- [ ] Non-functional requirements 있음
...
- **Data Model에 타입 없음**: 런타임 타입 정의 없으면 개발자가 매번 추측해야 한다.
- **Edge Case와 Test Case 중복**: Edge Case는 **동작 정의**, Test Case는 **검증 시나리오**. 역할이 다르다.
- **UI를 "알아서"로 처리**: 화면 요소를 나열해야 디자이너와 협업 가능.
- **Non-functional requirements 누락**: 성능 기준 없으면 최적화 논쟁이 끝없이 반복된다.
