...
description: Use to design a game's core gameplay loops at 30-second, 5-minute, and daily timescales, plus long-term progression. Triggered right after concept-brief is established, before rules or systems are designed. Produces core-loop.md that defines how player time is structured at every scale.
---

# Game Core Loop Design Skill
...
- **Pacing**: 긴장과 이완의 리듬. 5분 루프가 전부 "긴장"이면 플레이어가 지친다.
- **Choice density**: 의미 있는 선택이 루프당 몇 개 있는가. 0이면 자동재생과 다를 바 없다.
- **Asymmetric reward**: 좋은 플레이와 나쁜 플레이가 다른 보상을 받아야 한다. 완전 균일 보상은 성장 실감을 죽인다.
