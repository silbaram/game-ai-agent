...
플랫폼별 복사본은 `.claude/skills/`, `.agents/skills/`에 자동 동기화된다 (`scripts/sync-skills.sh`).
...
6. `spreadsheet-architect` → 데이터 테이블 생성
7. `balance-reviewer` → 숫자 검토 → 5~6단계 피드백
8. `game-rules-designer` → `game-system-spec` skill 실행 → 개발 명세 확정
9. (외부) Web LLM으로 컨셉 이미지 / UI mockup 생성
10. Claude Code / Codex / Gemini로 실제 구현

**절대 어기지 않는 규칙**: 4번 이후 반드시 5번을 거친다. MVP 축소 없이 구현에 들어가면 기획이 우주 규모로 부풀어 오른다.
...
- Codex/Gemini에서는 `SKILL.md`만 쓰고, agent 역할은 프롬프트로 주입
