...
이 하네스의 개발 흐름은 "아이디어를 바로 코드로 보내는 것"이 아니라, 기획을 검토 가능한 문서와 데이터로 고정한 뒤 구현으로 넘기는 방식입니다. 각 단계는 다음 단계의 입력 파일을 만들며, agent끼리 자동으로 이어지지 않습니다. 사람이 현재 단계의 산출물을 확인한 뒤 다음 agent를 호출합니다.

| 단계 | 목적 | Agent / Skill | 주요 산출물 | 다음 단계로 가는 조건 |
|---|---|---|---|---|
| 0 | 최소 입력 확보 | 사람 | 장르, 플랫폼, 핵심 재미, 타겟 플레이어 | 4개 필수값이 모두 있음 |
| 1 | 게임 뼈대 확정 | `game-director` + `game-concept-brief`, `game-core-loop-design` | `game-design/concept-brief.md`, `game-design/game-pillars.md`, `game-design/core-loop.md`, `game-design/system-overview.md` | 장르/플랫폼/루프/시스템 후보가 한 방향으로 정리됨 |
| 2 | 세계관과 시각 방향 정리 | `game-concept-designer` + `game-image-prompt-pack` | `game-design/art/*.md` | 아트 방향 없이 이미지 프롬프트를 만들지 않음 |
| 3 | 시스템 규칙 설계 | `game-rules-designer` + `game-rule-design` | `game-design/rules/*.md` | 각 규칙에 숫자, 입력, 결과, 실패 조건이 있음 |
| 4 | MVP 범위 축소 | `production-scope-reviewer` + `game-mvp-scope` | `game-design/mvp-scope.md`, `ai/reviews/production/*.md` | Must / Later / Cut이 분리됨 |
| 5 | 데이터 테이블화 | `spreadsheet-architect` + `game-spreadsheet-authoring` | `game-design/spreadsheets/*.csv`, `game-design/spreadsheets/game-master.xlsx` | MVP에 남은 규칙만 데이터화됨 |
| 6 | 숫자 검토 | `balance-reviewer` + `game-balance-review` | `ai/reviews/balance/*.md` | 성장, 보상, 가격, 드랍률 이상치가 기록됨 |
| 7 | 개발 명세 확정 | `game-rules-designer` + `game-system-spec` | `game-design/systems/*.md` | 구현자가 추측 없이 데이터 모델과 규칙을 읽을 수 있음 |
| 8 | UI 명세 작성 | `ui-planner` + `design-system-spec`, `game-screen-spec` | `ai/specs/ui/*.md` | 화면별 상태, 데이터 요구사항, 인터랙션이 정리됨 |
| 9 | 외부 시각 자료 생성 | Web LLM | 컨셉 이미지, UI mockup, 아이콘 시안, 무드보드 | 결과물을 `game-design/art/` 또는 `ai/specs/ui/`에 명세로 반영함 |
| 10 | 구현 | `ui-implementer` + `game-ui-implementation` | `src/*`, `components/*`, `screens/*`, `tests/*` | UI 명세와 시스템 명세가 존재함 |
| 11 | 브라우저 검토 | `browser-preview-reviewer` + `game-browser-preview-review` | `ai/reviews/visual/*.md` | Preview, 반응형, 빌드/테스트 결과가 기록됨 |

중요한 게이트:

- `game-director`는 장르, 플랫폼, 핵심 재미, 타겟 플레이어가 없으면 파일을 만들지 않고 먼저 질문해야 합니다.
- `game-rule-design` 이후에는 반드시 `game-mvp-scope`를 거칩니다.
- MVP 축소 없이 스프레드시트, 시스템 명세, 구현에 들어가지 않습니다.
- UI 구현은 `ai/specs/ui/design-system.md`와 화면별 `*-screen.md`가 생긴 뒤에만 시작합니다.
- Web LLM 결과물은 source of truth가 아닙니다. 구현 전에 반드시 문서 명세로 변환합니다.
...
| Agent | 역할 | 출력 위치 |
|---|---|---|
| `game-director` | 전체 방향, pillars, core loop, 시스템 목록 | `game-design/concept-brief.md`, `game-design/game-pillars.md`, `game-design/core-loop.md`, `game-design/system-overview.md` |
...
같은 역할을 호출하더라도 실제 런타임 구조는 도구마다 다릅니다. 하네스 원본은 하나지만, `sync-skills.sh`가 각 도구에 맞는 실행 파일로 변환합니다.
...
| Skill 위치 | `.claude/skills/<skill>/SKILL.md` | `.agents/skills/<skill>/SKILL.md` | `.gemini/skills/<skill>/SKILL.md`, `.agents/skills/<skill>/SKILL.md` |
| 호출 방식 | `Use game-director subagent...` | `game-director를 사용해서...` | `/agents:game-director ...` |
| 다음 agent 호출 | 사람이 직접 호출 | 사람이 직접 호출 | 사람이 직접 호출 |
| 입력 부족 시 | agent 지침에 따라 질문 | agent TOML 지침에 따라 질문 | command가 주입한 역할 지침에 따라 질문 |

따라서 README의 흐름은 모든 도구에 공통으로 적용되지만, 실제 파일 위치와 호출 문법은 다릅니다. 기능 차이를 줄이려면 하네스 원본을 수정한 뒤 대상 프로젝트에 다시 동기화합니다.
...
시작 전에 아래 4가지만 정하면 됩니다. 이 값들은 `game-director`의 필수 입력입니다. 하나라도 없으면 agent는 `game-design/` 파일을 만들지 않고 먼저 질문해야 합니다.
...
아래 값들은 선택 입력입니다. 비워 두면 agent가 초안을 만들 수 있지만, 입력하면 방향이 더 안정됩니다.
...
첫 요청에는 `game-director`를 사용한다고 명시하고, 필수 입력 4개를 함께 제공합니다. 필수 입력이 부족하면 agent는 저장소 이름이나 README에서 추측하지 않고 질문만 반환해야 합니다.
...
아직 아이디어가 흐릿하다면 아래처럼 질문을 요청해도 됩니다. 이 경우 파일은 생성되지 않고 입력표가 먼저 돌아와야 합니다.

```text
game-director를 사용해서 게임 뼈대 문서를 만들어줘.
```

도구별로 전체 문장을 다르게 외울 필요는 없습니다. 다만 Gemini CLI에서 custom command를 쓰고 싶으면 아래처럼 필수 입력을 함께 전달합니다.
...
| 순서 | 요청 | Agent | 주요 결과 |
|---|---|---|---|
| 1 | 게임 뼈대 문서 생성 | `game-director` | `game-design/concept-brief.md`, `game-design/game-pillars.md`, `game-design/core-loop.md`, `game-design/system-overview.md` |
...
| 9 | 컨셉 이미지와 UI mockup 생성 | Web LLM | `game-design/art/*.md`, `ai/specs/ui/*.md`에 반영할 시각 레퍼런스 |
| 10 | 실제 UI 구현 | `ui-implementer` | `src/*`, `components/*`, `screens/*`, `tests/*` |
| 11 | 브라우저 preview 검토 | `browser-preview-reviewer` | `ai/reviews/visual/*.md` |
...
- Web LLM 결과물은 바로 구현하지 않고 명세 문서에 반영한 뒤 사용합니다.
...
concept / art direction
  ↓
rules → MVP scope → spreadsheet → balance
  ↓
system spec → UI spec
  ↓
Web LLM 시각 자료를 명세에 반영
  ↓
ui-implementer 구현
  ↓
